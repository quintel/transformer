...
describe Transformer::Caster::GasConsumption do
  let(:dataset) { Atlas::Dataset::Derived.find(:ameland) }
  let(:analyzer) {
    Transformer::Caster::GasConsumption.analyze(dataset, nil, {
      'gas_consumption'      => 500,
  ...
  before do
    expect_any_instance_of(Transformer::Caster::GasConsumption)
