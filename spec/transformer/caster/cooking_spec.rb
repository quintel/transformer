...
describe Transformer::Caster::Cooking do
  let(:dataset) { Atlas::Dataset::Derived.find(:ameland) }
  let(:analyzes) {
    Transformer::Caster::Cooking.analyze(dataset, nil, {}, {
      households_final_demand_for_cooking_network_gas: 100,
      households_final_demand_for_cooking_electricity: 1000
    })
  }

  before do
    expect_any_instance_of(Transformer::Caster::Cooking)
